// Gestion UI et interactions temps réel
const {
  DARWIN_PENCILS,
  mapGrayToPencil,
  toGrayscale,
...
let graySnapshot = null; // ImageData en NB pour accès rapide
...
// Dessine la loupe en zoomant directement depuis le canvas NB
function renderMagnifier(clientX, clientY, sourceX, sourceY, zoom = 4) {
  const size = magCanvas.width; // 180
  const half = size / 2;
  ...
  // crosshair 1px
  ctxMag.strokeStyle = 'rgba(251,191,36,0.9)';
  ctxMag.lineWidth = 1;
  ctxMag.beginPath();
